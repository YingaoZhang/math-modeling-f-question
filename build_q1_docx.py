...
doc.add_paragraph('主要结论：同尺度 1M→1M 的逐域建模有效，CV 选型宏平均 R² 为 0.8896，纯指数模型宏平均 R² 为 0.8688；绝对 Loss 无法跨尺度迁移，秩在跨尺度上仍较稳定。质量分的冲突惩罚是无监督的定义型敏感性评分，因没有文档级 Q 与配方级 Loss 配对，不能宣称其带来监督增益。A12–A15 的估算数据表现出高度规则的乘性标度结构，应作为估算机制诊断而非实验结果。')
