...
doc.add_paragraph('主要结论：同尺度 1M→1M 的逐域建模有效，CV 选型宏平均 R² 为 0.8896，纯指数模型宏平均 R² 为 0.8688；绝对 Loss 不能直接跨尺度迁移，秩在跨尺度上仍较稳定。22 项质量信号均参与 ECDF、组内聚合与全量敏感性对照，主接口启用 rho=0.25 的非零冲突惩罚 Q*=clip(Q-rho C)，并以 rho=0/0.1/0.5/1.0 做稳健性界定。A12–A15 的估算数据表现出高度规则的乘性标度结构，应作为估算机制诊断而非实验结果。')
